...
namespace Laser.Orchard.StartupConfig.RazorCodeExecution.Activities {
    ...
    public class RazorExecuteActivity : Task {
        ...
        public override IEnumerable<LocalizedString> Execute(WorkflowContext workflowContext, ActivityContext activityContext) {
            var result = _razorExecuteService.Execute(activityContext.GetState<string>("RazorExecuteActivity_RazorView"), workflowContext.Content, workflowContext.Tokens, workflowContext).Trim();
            if (result == null) {
                result = "Error";
            } else if (result == "") {
                result = "Empty";
