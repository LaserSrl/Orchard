...
namespace Laser.Orchard.AppDirect {
    public class OAuthBase {
        ...
        private static List<OAuthBase.QueryParameter> GetQueryParameters(string parameters) {
            if (parameters.StartsWith("?"))
                parameters = parameters.Remove(0, 1);
            var queryParameterList = new List<OAuthBase.QueryParameter>();
        ...
        protected static string NormalizeRequestParameters(IList<OAuthBase.QueryParameter> parameters) {
            var stringBuilder = new StringBuilder();
        ...
        public string GenerateSignatureBase(Uri url, string consumerKey, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, string signatureType, out string normalizedUrl, out string normalizedRequestParameters) {
            ...
            normalizedRequestParameters = NormalizeRequestParameters((IList<OAuthBase.QueryParameter>)queryParameters);
            var stringBuilder = new StringBuilder();
        ...
        public string GenerateSignature(Uri url, string consumerKey, string consumerSecret, string token, string tokenSecret, string httpMethod, string timeStamp, string nonce, OAuthBase.SignatureTypes signatureType, out string normalizedUrl, out string normalizedRequestParameters) {
            ...
            switch (signatureType) {
                case OAuthBase.SignatureTypes.HMACSHA1:
                    var signatureBase = this.GenerateSignatureBase(url, consumerKey, token, tokenSecret, httpMethod, timeStamp, nonce, "HMAC-SHA1", out normalizedUrl, out normalizedRequestParameters);
                    string output;
                    using (HMACSHA1 hmacshA1 = new HMACSHA1()) {
                        ...
                        output = GenerateSignatureUsingHash(signatureBase, (HashAlgorithm)hmacshA1);
                    }
                    return output;
