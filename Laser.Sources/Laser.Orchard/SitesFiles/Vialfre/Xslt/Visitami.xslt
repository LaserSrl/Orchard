...
<xsl:template match="Event/MediaResource/Name">
	<xsl:call-template name="FiltroLingua"/>
</xsl:template>
<xsl:template match="Event/Venue/Name">
	<xsl:call-template name="FiltroLingua"/>
</xsl:template>
...
<xsl:template match="Schedule">
	<xsl:element name="Schedule">
        ...
        <!-- >xsl:if test="concat($day,'/',$month,'/',$year) = $startdate" -->	
					<xsl:element name="{local-name()}" namespace="">
            <xsl:apply-templates select="node()"/> 
					</xsl:element>
					<!--/xsl:if -->
...
<xsl:template match="Event">
	<xsl:if test="not(preceding-sibling::*[1][self::Event])">
		<xsl:text disable-output-escaping="yes">	<![CDATA[<ToRemove>]]></xsl:text>
	</xsl:if>
	<xsl:element name="Event" namespace="">
		<xsl:apply-templates select="node()"/>
	</xsl:element>
	<xsl:if test="not(following-sibling::*[1][self::Event])">
		<xsl:text disable-output-escaping="yes"><![CDATA[<Event></Event></ToRemove>]]></xsl:text>
	</xsl:if>	
</xsl:template>
</xsl:stylesheet>